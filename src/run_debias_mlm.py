...
def train(args, data, datasets, model: PreTrainedModel, original_model, tokenizer: PreTrainedTokenizer, attributes_definitions: list) -> Tuple[int, float]:
    """ Train the model """
    ...
    def attribute_vector_definition():
        # gender vectorを計算
        attributes_hiddens = {f'attribute{i}': [] for i in range(len(attributes_definitions))}
        for i, attribute_definitions in enumerate(attributes_definitions):
            for definition in attributes_definitions:
                definition = tokenizer.encode(definition)
                definition = torch.tensor(definition, requires_grad=False).to(args.device)
                definition = torch.unsqueeze(definition, 0)
                hiddens = get_hiddens_of_model(definition)
                if args.model_type == 'roberta' or args.model_type == 'bert' or args.model_type == 'electra' or args.model_type == 'albert' or args.model_type == 'dbert':
                    attributes_hiddens[f'attribute{i}'] += [torch.stack([hidden[:,0] for hidden in hiddens]).squeeze()]
                elif args.model_type == 'gpt2' or args.model_type == 'gpt':
                    attributes_hiddens[f'attribute{i}'] += [torch.stack([hidden[:,-1] for hidden in hiddens]).squeeze()]

            attributes_hiddens[f'attribute{i}'] = torch.mean(torch.stack(attributes_hiddens[f'attribute{i}']), 0)
            attributes_hiddens[f'attribute{i}'] = attributes_hiddens[f'attribute{i}'].detach().unsqueeze(0)

        return attributes_hiddens

    def attribute_vector_definition_example():
        pass
...
def main():
    ...
    if 'attributes_definitions' in data:
        attributes_definitions = data['attributes_definitions']
    ...
    train(args, splited_data, datasets, model, original_model, tokenizer, attributes_definitions)
